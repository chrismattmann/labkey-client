...
public class Demo {
	...
	public void sortFilterDemo() throws Exception {
		...
		SelectRowsCommand cmd = new SelectRowsCommand("lists", "Technicians");

		// sort by name ascending
		cmd.addSort("LastName", Sort.Direction.ASCENDING);
		SelectRowsResponse resp = cmd.execute(cn, "HIV-CD4 Study");

		for (Map<String, Object> row : resp.getRows()) {
			System.out.println("Name is "+row.get("FirstName") + " " + row.get("LastName"));
		}
	}
	...
	public void executeSqlDemo() throws Exception {
		...
		cmd.setSql("select Label from Visit");
		cmd.setTimeout(0);

		SelectRowsResponse resp = cmd.execute(cn, "HIV-CD4 Study");

		System.out.println("Label is "
				+ resp.getRows().get(0).get("Label"));
	}
	...
	public void getWebPartDemo() throws Exception {
		...
		cmd.getParameters().put("name", "default");

		CommandResponse resp = cmd.execute(cn, "HIV-CD4 Study");
	...
	public Demo(String[] args) throws Exception {
		processArgs(args);
		if (null == _email || null == _password || null == _connectionUrl)
			throw new Exception(
					"Usage: java demo.class <user> <password> <url>");
	}
	...
	private void processArgs(String[] args) {
		if (args.length >= 2) {
			_email = args[0];
			_password = args[1];
			_connectionUrl = args[2];
		}
	}

	private Connection getConnection() {
		if (null == _connection)
			_connection = new Connection(_connectionUrl, _email, _password);
		return _connection;
	}

	private String _email = null;
	private String _password = null;
	private String _connectionUrl = null; // "http://localhost:8080/labkey"
